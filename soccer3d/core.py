    ...
    def estimate_poses(self, redo=False, openpose_dir='/path/to/openpose', pad=150):
        ...
        if exists(pose_file_coarse) and not redo:
            ...
            for i, basename in enumerate(tqdm(self.frame_basenames)):
                ...
                command = '{0} --model_pose COCO --net_resolution 200x-1 --image_dir {1} --write_json {2} --model_folder {3} --display 0 --render_pose 0'.format(openposebin, tmp_dir, tmp_dir, openpose_dir+'models')
